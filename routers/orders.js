...
router.post('/', async (req, res) => {
    ...
    if (!order) {
        res.status(400).send('order can"t be created')
    }
...
router.put('/:id', async (req, res) => {
    ...
    if (!order) {
        return res.status(400).send({ message: 'order can"t be updated' })
    }
    res.status(200).send(order);
})
...
router.get('/get/totalsales', async (req, res) => {
    ...
    if (!totalSales) {
        return res.status(400).send('order sales can"t be generated')
    }
...
router.get(`/get/userorders/:userid`, async (req, res) => {
    const userorderList = await Order.find({ user: req.body.userid })
        .populate({ path: 'orderItems', populate: { path: 'product', populate: 'category' } })
        .sort({ 'dateOrdered': -1 });
    if (!userorderList) {
        res.send(500).json({ success: false })
